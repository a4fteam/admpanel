...
<div class="col-sm-12  col-md-3 col-lg-3">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form class="form-horizontal" method="POST" action="{{  $save_url }}" name="form"  enctype="multipart/form-data">
                {!! csrf_field() !!}
                <input type="hidden" name="_method" value="put"/>
                <div class="form-group">
                    <label class="col-xs-3 control-label">Название категории</label>
                    <div class="col-xs-9">
                        <input class="form-control" placeholder="Название категории" value="{{ $category->name or old('name', '') }}"  name="name" type="text" id="name" size="30" maxlength="20">
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-xs-3 control-label">Краткое описание</label>
                    <div class="col-xs-9">
                        <textarea class="form-control" placeholder="Краткое описание категории" name="description" id="description" cols="85" rows="7">{{ $category->description or old('description', '') }}</textarea>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-xs-3 control-label">Title(SEO)</label>
                    <div class="col-xs-9">
                        <input class="form-control" placeholder="" name="seo_title" id="meta_title" value="{{ $category->seo_title or old('seo_title', '') }}">
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-xs-3 control-label">Description(SEO)</label>
                    <div class="col-xs-9">
                        <textarea class="form-control" placeholder="" name="seo_description" id="meta_description">{{ $category->seo_description or old('seo_description', '') }}</textarea>
                    </div>
                </div>
                <div class="form-group">
                    <label class="col-xs-3 control-label">Keywords(SEO)</label>
                    <div class="col-xs-9">
                        <input class="form-control" placeholder="" name="seo_keywords" id="meta_keywords" value="{{ $category->seo_keywords or old('seo_keywords', '') }}">
                    </div>
                </div>
                <div class="form-group">
